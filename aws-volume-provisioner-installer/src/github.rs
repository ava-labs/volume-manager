    ...
    let file_name = {
        if os.is_none() {
            if cfg!(target_os = "macos") {
                format!("aws-volume-provisioner.{arch}-apple-darwin")
            } else if cfg!(unix) {
            ...
            let os = os.unwrap();
            match os {
                Os::MacOs => format!("aws-volume-provisioner.{arch}-apple-darwin"),
                Os::Linux => format!("aws-volume-provisioner.{arch}-unknown-linux-gnu"),
                Os::Ubuntu20 => {
